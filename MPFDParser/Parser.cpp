...
MPFD::Parser::Parser() {
	...
	FilenameGeneratorUserData = NULL;
}
...
void MPFD::Parser::SetFilenameGenerator(MPFD::Parser::FilenameGenerator_t func, void *userdata)
{
    ...
    if (name_pos == std::string::npos) {
        ...
        if (filename_pos == std::string::npos) {
            ...
            if (content_type_pos != std::string::npos) {
                unsigned long content_type_end_pos = 0;
                for (unsigned long i = content_type_pos + 14; (i < headers.length()) && (!content_type_end_pos); i++) {
